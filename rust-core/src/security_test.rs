...
mod tests {
    ...
    async fn test_file_encryption_decryption() {
        ...
        // Encrypt
        let encrypted = manager
            .encrypt_file_data(session_id, original_data)
            .await
            .unwrap();
        assert_ne!(encrypted.ciphertext, original_data.to_vec());
        ...
        // Decrypt
        let decrypted = manager
            .decrypt_file_data(session_id, &encrypted)
            .await
            .unwrap();
        assert_eq!(decrypted, original_data.to_vec());
    }
    ...
    async fn test_signaling_encryption_decryption() {
        ...
        // Encrypt
        let encrypted = manager
            .encrypt_signaling_data(session_id, original_data)
            .await
            .unwrap();
        assert_ne!(encrypted.ciphertext, original_data.to_vec());
        ...
        // Decrypt
        let decrypted = manager
            .decrypt_signaling_data(session_id, &encrypted)
            .await
            .unwrap();
        assert_eq!(decrypted, original_data.to_vec());
    }
    ...
    async fn test_expired_certificate_validation() {
        ...
        let result = manager
            .validate_device_certificate(&expired_cert)
            .await
            .unwrap();
        assert!(!result.is_valid);
        assert!(result
            .validation_errors
            .contains(&CertificateValidationError::Expired));
    }
    ...
    async fn test_revoked_certificate_validation() {
        ...
        assert!(result
            .validation_errors
            .contains(&CertificateValidationError::Revoked));
    }
